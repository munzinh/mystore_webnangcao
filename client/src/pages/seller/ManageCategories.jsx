...
const ManageCategories = () => {
    ...
    const allRoots = categories.filter(c => !getParentId(c));
    const childMap = categories.reduce((map, cat) => {
        const parentId = getParentId(cat);
        if (!parentId) return map;
        return { ...map, [parentId]: [...(map[parentId] || []), cat] };
    }, {});

    const groupedCategories = allRoots
        .map(root => {
            const children = childMap[root._id] || [];
            if (!normalizedSearch) return { root, children };

            const rootMatches = root.name.toLowerCase().includes(normalizedSearch);
            const matchedChildren = children.filter(child => child.name.toLowerCase().includes(normalizedSearch));
            if (!rootMatches && matchedChildren.length === 0) return null;

            return { root, children: rootMatches ? children : matchedChildren };
        })
        .filter(Boolean);

    const orphanChildren = categories.filter(cat => {
        const parentId = getParentId(cat);
        if (!parentId) return false;
        const hasParent = categories.some(parent => parent._id === parentId);
        const matchesSearch = !normalizedSearch || cat.name.toLowerCase().includes(normalizedSearch);
        return !hasParent && matchesSearch;
    });

    const visibleCount = groupedCategories.reduce((sum, group) => sum + 1 + group.children.length, 0) + orphanChildren.length;
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-2.5 text-sm font-medium text-gray-600 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition shadow-sm"
                    >
                    ...
                    <div className="divide-y divide-gray-100">
                        ...
                        {orphanChildren.length > 0 && (
                            <div className="px-4 py-4 bg-amber-50/60">
                                <p className="text-xs font-bold uppercase tracking-wide text-amber-700 mb-2">Danh mục con chưa có cha</p>
...
const AttributeChips = ({ attributes = [] }) => {
    if (!attributes.length) {
        return <span className="text-xs text-gray-400">Chưa cấu hình</span>;
    }
    ...
    return (
        <div className="flex flex-wrap gap-1">
            ...
            {attributes.length > 3 && (
                <span className="text-gray-400 text-xs">+{attributes.length - 3}</span>
            )}
        </div>
    );
};

const StatusButton = ({ cat, onToggle }) => (
...
const ActionButtons = ({ cat, onEdit, onDelete }) => (
...
const CategoryGroup = ({ root, childrenCategories, onEdit, onDelete, onToggle }) => (
    <div className="px-4 py-3">
        <div className="grid grid-cols-[1fr_auto_auto] items-center gap-3 rounded-xl bg-gray-50 px-4 py-3">
            <div className="min-w-0">
                <div className="flex items-center gap-2">
                    <span className="h-2 w-2 rounded-full bg-indigo-500" />
                    <span className="font-bold text-gray-800 truncate">{root.name}</span>
                    <span className="rounded-full bg-white px-2 py-0.5 text-xs font-medium text-gray-500 border border-gray-100">
                        {childrenCategories.length} danh mục con
                    </span>
                </div>
                <div className="mt-1 flex items-center gap-3 text-xs text-gray-500">
                    <span>Danh mục gốc</span>
                    <AttributeChips attributes={root.suggestedAttributes || []} />
                </div>
            </div>
            <StatusButton cat={root} onToggle={onToggle} />
            <ActionButtons cat={root} onEdit={onEdit} onDelete={onDelete} />
        </div>

        {childrenCategories.length > 0 ? (
            <div className="mt-2 ml-5 overflow-hidden rounded-xl border border-gray-100">
                <div className="grid grid-cols-[1.2fr_1fr_auto_auto] bg-white px-4 py-2 text-xs font-semibold uppercase text-gray-400 border-b border-gray-100">
            ...
            <div className="ml-5 mt-2 rounded-xl border border-dashed border-gray-200 px-4 py-2 text-xs text-gray-400">
...
const CategoryChildRow = ({ cat, onEdit, onDelete, onToggle }) => (
    <div className="grid grid-cols-[1.2fr_1fr_auto_auto] items-center gap-3 px-4 py-2.5 text-sm hover:bg-indigo-50/40 transition border-b border-gray-50 last:border-b-0">
        <div className="min-w-0">
            <p className="font-semibold text-gray-700 truncate">{cat.name}</p>
            <p className="text-xs text-gray-400">Thuộc {cat.parentId?.name || 'danh mục cha'}</p>
        </div>
        <AttributeChips attributes={cat.suggestedAttributes || []} />
        <div className="text-center">
            <StatusButton cat={cat} onToggle={onToggle} />
        </div>
        <ActionButtons cat={cat} onEdit={onEdit} onDelete={onDelete} />
    </div>
);
